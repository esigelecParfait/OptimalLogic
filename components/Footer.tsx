        ...
        <div className="mb-12 flex flex-col gap-11 md:flex-row md:items-start md:justify-between">
          <div>
            <Link href="/" className="flex items-center gap-3">
              <span
                className="grid h-10 w-10 place-items-center rounded-xl font-display text-base font-bold text-black"
              ...
              <div className="flex flex-wrap items-center gap-x-6 gap-y-2 md:justify-end">
                {col.links.map((link) => (
                  <Link
                    key={link.label}
                    href={link.href}
                    className="text-[14.5px] text-mut transition-colors hover:text-ink"
                  >
                    {link.label}
                  </Link>
                ))}
