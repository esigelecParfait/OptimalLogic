...
const COMMAND_TYPES = [
  {
    value: "gateau_anniversaire",
    label: "Gâteau d'anniversaire",
    desc: "Personnalisé, à partir de 35 €",
  },
  {
    value: "piece_montee",
    label: "Pièce montée mariage",
    desc: "Sur devis, consultation gratuite",
  },
  {
    value: "buffet_patissier",
    label: "Buffet pâtissier",
    desc: "Entreprise ou événement, à partir de 80 €",
  },
  {
    value: "pain_special",
    label: "Pain sans gluten",
    desc: "Sur commande 48h à l'avance",
  },
  {
    value: "consultation",
    label: "Consultation / RDV",
    desc: "Pour un projet sur-mesure",
  },
...
const fieldClass =
  "h-12 w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 text-sm text-slate-950 outline-none transition placeholder:text-slate-400 focus:border-slate-950 focus:bg-white";
...
export default function RDVBooking() {
    ...
    return (
      <div className="mx-auto max-w-lg py-12 text-center">
        ...
        <p className="mt-3 text-sm leading-7 text-slate-600">
          L’équipe des Délices de Léon va vous recontacter sous 24h pour confirmer les
          détails.
        </p>
        <p className="mt-2 text-sm text-slate-500">
          Un email de confirmation vous a été envoyé à <strong>{prospectEmail}</strong>.
        </p>
        <p className="mt-1 text-xs text-slate-400">
          Référence : {confirmedId.slice(0, 8).toUpperCase()}
        </p>
  ...
  return (
    <div className="mx-auto max-w-lg">
      {step === "type" && (
        <div>
          <h2 className="text-xl font-bold tracking-tight text-slate-950 mb-6">
            Quel type de commande souhaitez-vous ?
          </h2>
      ...
      {step === "details" && (
        <div>
          <button
            ...
            className="mb-6 flex items-center gap-1.5 text-sm text-slate-500 hover:text-slate-950"
          >
            <svg
              width="14"
              height="14"
          ...
          <div className="grid gap-5">
              ...
              <div>
                ...
                {loadingSlots ? (
                  ...
                  <p className="text-sm text-slate-500">
                    Aucun créneau disponible. Nous vous contacterons pour convenir d’un
                    horaire.
                  </p>
                ) : (
                  <select
                    ...
                    onChange={(e) => {
                      const slot = slots.find(
                        (s) => `${s.date}|${s.startTime}` === e.target.value,
                      );
                      if (slot) {
                        setDateSouhaitee(slot.date);
                        setHeureRdv(slot.startTime);
                      }
                    }}
                    className={fieldClass}
                  >
            ...
            {(typeCommande === "gateau_anniversaire" ||
              typeCommande === "buffet_patissier" ||
              typeCommande === "piece_montee") && (
              <div>
            ...
            <div>
              ...
              <textarea
                ...
                placeholder="Saveurs, couleurs, thème, allergies..."
                rows={4}
                maxLength={1000}
                className="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-950 outline-none transition placeholder:text-slate-400 focus:border-slate-950 focus:bg-white resize-none"
              />
            ...
            <button
              ...
              disabled={!dateSouhaitee}
              className="h-12 rounded-full bg-slate-950 text-sm font-semibold text-white transition hover:bg-slate-800 disabled:cursor-not-allowed disabled:opacity-50"
            >
              Continuer
            </button>
          </div>
          ...
          <div className="grid gap-5">
            ...
            <label className="flex items-start gap-3 cursor-pointer">
              ...
              <span className="text-xs leading-5 text-slate-500">
                J’accepte que mes données soient utilisées par Les Délices de Léon pour
                traiter ma demande, conformément au RGPD.
              </span>
            </label>
            {error && (
            ...
            <button
              ...
              disabled={!prospectNom || !prospectEmail || !consentement || submitting}
              className="h-12 rounded-full bg-slate-950 text-sm font-semibold text-white transition hover:bg-slate-800 disabled:cursor-not-allowed disabled:opacity-50"
            >
