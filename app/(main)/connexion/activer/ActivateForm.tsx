    ...
    <div className="mt-2 space-y-1">
      <div className="flex gap-1">
        {[0, 1, 2, 3].map((i) => (
          <div
            key={i}
            className={`h-1 flex-1 rounded-full transition-all ${i < score ? colors[score - 1] : "bg-white/[0.08]"}`}
          />
        ))}
      </div>
      <p
        className={`text-[11px] font-medium ${score <= 1 ? "text-[#ff6b6b]" : score <= 3 ? "text-amber-400" : "text-emerald"}`}
      >
  ...
  if (token && error && !tokenEmail) {
    return (
      <div className="mx-auto w-full max-w-[420px] text-center">
        ...
        <a
          href="/contact"
          className="mt-5 inline-block text-sm text-mut underline underline-offset-2 hover:text-ink"
        >
    ...
    <div className="mx-auto w-full max-w-[420px]">
      <div className="mb-8 text-center">
        ...
        <p className="mt-2 text-sm text-mut">
          {tokenEmail
            ...
            : "Entrez votre e-mail et choisissez un mot de passe."}
        </p>
      </div>
      ...
      <div className="surface-card rounded-2xl p-7">
        <form onSubmit={handleSubmit} className="space-y-5">
            ...
            {tokenEmail ? (
              <div className="flex h-12 items-center gap-3 rounded-xl border border-white/[0.07] bg-white/[0.03] px-4">
                <svg
                  ...
                  stroke="currentColor"
                  strokeWidth="2"
                  className="flex-shrink-0 text-mut-2"
                >
          ...
          {tokenEmail && (
            <p className="text-center text-[11px] text-mut-2">
              Ce lien est valable 2 heures et à usage unique.
            </p>
          )}
        </form>
      </div>
      ...
      <p className="mt-5 text-center text-[11px] text-mut-2">
        ...
        <a
          href="/connexion"
          className="text-mut underline underline-offset-2 hover:text-ink"
        >
          Se connecter
        </a>
      </p>
