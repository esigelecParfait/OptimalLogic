...
export default async function AdminDashboardPage() {
  ...
  const [prospectsRes, clientsRes, demandesRes, newThisMonthRes, recentDemandesRes] =
    await Promise.all([
      supabaseAdmin
      ...
      supabaseAdmin
        ...
        .in("request_status", ["nouveau", "a_contacter"]),
      supabaseAdmin
        .from("client_prospects")
        .select("id_client", { count: "exact", head: true })
      ...
      supabaseAdmin
        .from("demandes")
        .select(
          "id, request_status, request_source, offer_code, created_at, id_client, client_prospects(contact_first_name, contact_last_name, business_name, type_client)",
        )
    ...
    <div className="p-8 space-y-8">
      ...
      <div className="grid grid-cols-3 gap-4">
        {stats.map((s) => (
          <Link
            key={s.label}
            href={s.href}
            className="surface-card rounded-2xl p-5 hover:bg-white/[0.03] transition-colors"
          >
            <p className="text-xs font-medium text-mut-2 uppercase tracking-wider mb-3">
              {s.label}
            </p>
            <p
              className={`font-display text-4xl font-semibold ${s.color} leading-none mb-2`}
            >
              {s.value}
            </p>
      ...
      <section className="surface-card rounded-2xl overflow-hidden">
        ...
        <table className="w-full text-sm">
          <tbody className="divide-y divide-white/[0.04]">
            {recentDemandes.map((d) => {
              const p = Array.isArray(d.client_prospects)
                ? d.client_prospects[0]
                : d.client_prospects;
              const name =
                p?.business_name ||
              ...
              return (
                <tr key={d.id} className="hover:bg-white/[0.02] transition-colors">
                  <td className="px-5 py-3.5">
                    ...
                    {p?.type_client && (
                      <p className="text-xs text-mut-2 mt-0.5">
                  ...
                  <td className="px-5 py-3.5">
                    <span
                      className={`rounded-full border px-2.5 py-1 text-[11px] font-medium ${statusColors[status] ?? "text-mut border-white/10"}`}
                    >
                      {status.replace(/_/g, " ")}
                    </span>
                  </td>
                  <td className="px-5 py-3.5 text-xs text-mut capitalize">
                    {d.request_source ?? "—"}
                  </td>
                  <td className="px-5 py-3.5 text-xs text-mut text-right">
                    {formatRequestDate(d.created_at)}
                  </td>
                </tr>
              );
            })}
        ...
        {recentDemandes.length === 0 && (
          <div className="py-12 text-center text-sm text-mut">
