...
function findBestMatch(
  locationName: string,
  clients: DbClient[],
): { client: DbClient; score: number } | null {
...
export default function GoogleSetupClient({ clients }: { clients: DbClient[] }) {
  ...
  const linkOne = useCallback(
    async (loc: GBPLocation, clientId: string): Promise<boolean> => {
    ...
    return (
      <div className="surface-card rounded-2xl p-8">
        <p className="font-semibold text-[#ff6b6b]">
          Impossible de se connecter à Google Business
        </p>
        <p className="mt-1 text-sm text-mut">{error}</p>
        <button
          onClick={loadLocations}
          className="mt-4 btn-ghost rounded-full px-5 py-2 text-sm font-semibold"
        >
          Réessayer
        </button>
      </div>
    ...
    <div className="grid gap-5">
      <div className="surface-card rounded-2xl p-5 flex items-center justify-between gap-4 flex-wrap">
          ...
          {[
            ...
            <div key={i} className="flex items-center gap-6">
              ...
              <div className="text-center">
                <p
                  className={`font-display text-2xl font-semibold ${stat.color ?? "text-ink"}`}
                >
                  {stat.val}
                </p>
        ...
        <div className="flex items-center gap-3">
          {autoResult && (
            <span
              className={`text-sm font-semibold ${autoResult.startsWith("✓") ? "text-emerald-400" : "text-amber-400"}`}
            >
              {autoResult}
            </span>
          )}
              ...
              return (
                <div key={loc.name} className="flex items-center gap-3 px-5 py-3.5">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {loc.primaryCategory && (
                      <p className="text-[11px] text-mut-2 truncate">
                        {loc.primaryCategory}
                      </p>
                    )}
                  </div>
                  <select
                    value={selected}
                    onChange={(e) =>
                      setSelections((s) => ({ ...s, [loc.name]: e.target.value }))
                    }
