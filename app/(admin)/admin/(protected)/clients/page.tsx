...
export default async function ClientsPage({
  ...
  const [clientsRes, servicesRes] = await Promise.all([
    supabaseAdmin
      .from("clients")
      .select("id_client, status, became_client_at, id_client_prospect")
      .order("became_client_at", { ascending: false }),
    supabaseAdmin
      .from("client_services")
      .select("id_service, id_client, offer_code, service_status, payment_status"),
  ]);

  const clients: ClientRow[] = clientsRes.data ?? [];
  const services: ServiceRow[] = servicesRes.data ?? [];

  const prospectIds = clients
    .map((c) => c.id_client_prospect)
  ...
  const prospectsRes =
    prospectIds.length > 0
      ? await supabaseAdmin
          ...
          .select(
            "id_client, contact_first_name, contact_last_name, contact_email, business_name, type_client",
          )
  ...
  const emails = (prospectsRes.data ?? [])
  ...
  const filteredClients = query
    ? clients.filter((c) => {
        const p = c.id_client_prospect
          ? prospectMap.get(c.id_client_prospect)
          : undefined;
        const haystack = [
  ...
  const payBadge = (s: string) => (s === "paye" ? "text-emerald-400" : "text-yellow-400");

  const tokenBadge = (t: TokenRow | undefined) => {
    if (!t)
      return {
        label: "Aucun lien",
        cls: "text-mut-2 bg-white/[0.04] border-white/[0.08]",
      };
    if (t.used_at)
      return {
        label: "Activé",
    ...
    if (new Date(t.expires_at) < new Date())
      return { label: "Expiré", cls: "text-red-400 bg-red-400/10 border-red-400/20" };
    return {
      label: "Lien actif",
    ...
    <div className="p-8 space-y-10">
      ...
      <section className="surface-card rounded-2xl p-6">
        <h2 className="font-display text-base font-semibold text-ink mb-1">
          Générer un lien d&apos;accès (manuel)
        </h2>
        <p className="text-xs text-mut mb-4">
          Pour copier le lien toi-même plutôt que de l&apos;envoyer automatiquement. Le
          client doit être actif avec un service payé en cours.
        </p>
        <GenerateLinkForm />
      </section>
          ...
          <tbody className="divide-y divide-white/[0.04]">
            {filteredClients.map((c) => {
              const p = c.id_client_prospect
                ? prospectMap.get(c.id_client_prospect)
                : undefined;
              const svcList = c.id_client_prospect
                ? (servicesByProspect[c.id_client_prospect] ?? [])
                : [];
              const token = p?.contact_email
                ? latestTokenByEmail.get(p.contact_email)
                : undefined;
                ...
                <tr key={c.id_client} className="hover:bg-white/[0.02] transition-colors">
                  <td className="px-5 py-4">
                    <p className="font-medium text-ink">
                      {p ? `${p.contact_first_name} ${p.contact_last_name}` : "—"}
                    </p>
                    <p className="text-xs text-mut mt-0.5">{p?.contact_email ?? "—"}</p>
                    {p?.business_name && (
                      <p className="text-xs text-mut-2 mt-0.5">{p.business_name}</p>
                    )}
                  </td>
                  <td className="px-5 py-4">
                    <span
                      className={`rounded-full border px-2.5 py-1 text-[11px] font-medium ${statusBadge(c.status)}`}
                    >
                      {c.status}
                    </span>
                  </td>
                  <td className="px-5 py-4">
                    {svcList.length === 0 ? (
                      <span className="text-mut text-xs">Aucun</span>
                    ) : (
                      <div className="space-y-1">
                        {svcList.map((s) => (
                          <div key={s.id_service} className="text-xs">
                            ...
                            {" · "}
                            <span className={payBadge(s.payment_status)}>
                              {s.payment_status}
                            </span>
                            {" · "}
                  ...
                  <td className="px-5 py-4">
                    <span
                      className={`rounded-full border px-2.5 py-1 text-[11px] font-medium ${tb.cls}`}
                    >
                      {tb.label}
                    </span>
                  </td>
