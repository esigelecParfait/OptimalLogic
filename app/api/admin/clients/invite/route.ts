  ...
  try {
    ...
    if (prospectError) {
      return Response.json(
        { error: "Impossible de verifier le client." },
        { status: 500 },
      );
    }
    ...
    if (!userId) {
      ...
      userId =
        users.find((authUser) => authUser.email?.toLowerCase() === email)?.id ?? null;
    }

    if (!userId) {
      return Response.json(
        { error: "Compte Auth client introuvable apres invitation." },
        { status: 500 },
      );
    }
