...
export async function POST(request: Request) {
  try {
    ...
    const requestSource = demande.request_source || "tarifs";
    ...
    if (requestSource !== "tarifs" && requestSource !== "contact") {
      return jsonError("La provenance de la demande est invalide.");
    }

    if (requestSource === "tarifs" && !offerCode) {
      return jsonError("Une demande provenant de la page Tarifs doit contenir une offre.");
    }

    if (requestSource === "tarifs" && !objectiveType) {
    ...
    console.log("offerCode reçu dans API :", offerCode);
    if (offerCode) {
      ...
      if (!offer) {
        console.error("Offre introuvable ou inactive :", {
          offerCode,
        });

        return jsonError(
          `L’offre choisie est invalide ou inactive. Code reçu : ${offerCode}`
        );
}
    }
